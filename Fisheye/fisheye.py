...
class Fisheye:
    def __init__(self, distance_to_object_cm=7., field_of_view_radius_cm=30., projection_type='seterographic'):
        print(f"Array size: {array_size_x_mm:.2f} mm x {array_size_y_mm:.2f} mm")
    ...
    def CalculateFocalLength(self):
        ...
        if self.projection_type == 'equidistant':
            self.focal_length_cm = self.sensor.max_image_radius_mm / np.radians(self.field_of_view_theta)
        elif self.projection_type == 'stereographic':
            self.focal_length_cm = self.sensor.max_image_radius_mm / (2 * np.tan(self.field_of_view_theta / 2))
        elif self.projection_type == 'orthographic':
            self.focal_length_cm = self.sensor.max_image_radius_mm / np.sin(self.field_of_view_theta)
        elif self.projection_type == 'equisolid_angle':
            self.focal_length_cm = self.sensor.max_image_radius_mm / (2 * np.sin(self.field_of_view_theta / 2))
    ...
    def ProjectPoints(self):
        ...
        thetas = np.arctan2(np.sqrt(points_3d[:, 0]**2 + points_3d[:, 1]**2), points_3d[:, 2])
        # Then, I need to compute the radius on the sensor plane for each point, based on the projection type.
        if self.projection_type == 'equidistant':
            radii_cm = self.focal_length_cm * thetas
        elif self.projection_type == 'stereographic':
            radii_cm = 2 * self.focal_length_cm * np.tan(thetas / 2)
        elif self.projection_type == 'orthographic':
            radii_cm = self.focal_length_cm * np.sin(thetas)
        elif self.projection_type == 'equisolid_angle':
            radii_cm = 2 * self.focal_length_cm * np.sin(thetas / 2)
        ...
        phis = np.arctan2(points_3d[:, 1], points_3d[:, 0])
        # The coordinates on the sensor plane are then:
        x_coords = radii_cm * np.cos(phis)
        y_coords = radii_cm * np.sin(phis)
        # Make image_points_2d_cm a Nx2 array
        self.image_points_2d_cm = np.column_stack((x_coords, y_coords))
...
class Sensor:
    def __init__(self, pixel_size_mm=0.01, num_pixels_x=1024, num_pixels_y=1024):
        ...
        self.max_image_radius_mm = np.min(self.array_size_x, self.array_size_y) / 2
